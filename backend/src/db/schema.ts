import { decimal, index, mysqlEnum, mysqlTable, tinyint, timestamp, uniqueIndex, varchar } from 'drizzle-orm/mysql-core';
...
export const users = mysqlTable('users', {
  ...
  role: mysqlEnum('role', ['ADMIN', 'BTC_FSTYLE', 'MC', 'MEMBER']).notNull().default('MEMBER'),
...
export const judgeScores = mysqlTable(
  'judge_scores',
  {
    ...
    ideaConcept: decimal('idea_concept', { precision: 4, scale: 1 }).default('0'), // max 20
    choreography: decimal('choreography', { precision: 4, scale: 1 }).default('0'), // max 25
    synchronization: decimal('synchronization', { precision: 4, scale: 1 }).default('0'), // max 20
    performance: decimal('performance', { precision: 4, scale: 1 }).default('0'), // max 20
    costume: decimal('costume', { precision: 4, scale: 1 }).default('0'), // max 10
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
  },
  (table) => [uniqueIndex('judge_team_idx').on(table.teamId, table.judgeNumber)],
);

// ── BTC Scores (category 6 — Kỷ luật, max 5đ) ────────
export const btcScores = mysqlTable('btc_scores', {
  id: varchar('id', { length: 36 }).primaryKey().$defaultFn(() => crypto.randomUUID()),
  teamId: varchar('team_id', { length: 36 }).notNull().references(() => teams.id).unique(),
...
export const effortVotes = mysqlTable(
  'effort_votes',
  {
    ...
    candidateId: varchar('candidate_id', { length: 36 }).notNull().references(() => users.id),
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => [
