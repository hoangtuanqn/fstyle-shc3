...
async function seed() {
  ...
  // ── 1. Teams ───────────────────────────────────────
  const teamData = [
    { id: crypto.randomUUID(), name: 'SHIRO KURO', concept: 'EMPTINESS', color: '#d0d0d0', displayOrder: 1 },
    { id: crypto.randomUUID(), name: 'Apex Aura', concept: 'INNER CONFLICT', color: '#D04047', displayOrder: 2 },
    { id: crypto.randomUUID(), name: 'SLATT', concept: 'AWAKENING', color: '#5973B3', displayOrder: 3 },
    { id: crypto.randomUUID(), name: 'ANTI-X', concept: 'LETTING GO', color: '#5EAF7C', displayOrder: 4 },
  ] as const;
  ...
  // ── 2. Users ───────────────────────────────────────
  const adminUsers = [
    { name: 'Phạm Hoàng Tuấn', email: 'phamhoangtuanqn@gmail.com', role: 'ADMIN' as const },
    { name: 'Nguyễn Hoàng Minh', email: 'nguyenminh150906@gmail.com', role: 'ADMIN' as const },
  ];

  const btcFstyleUsers = [
    { name: 'Nguyễn Thế Hiển', email: 'nguyenthehien28062005@gmail.com', role: 'BTC_FSTYLE' as const },
    { name: 'Nguyễn Hoàng Gia Phúc', email: 'titangiaphuc1804@gmail.com', role: 'BTC_FSTYLE' as const },
    { name: 'Nguyễn Thanh Lâm', email: 'lam01052004@gmail.com', role: 'BTC_FSTYLE' as const },
    { name: 'Phạm Hải Yến', email: 'haiin260606@gmail.com', role: 'BTC_FSTYLE' as const },
  ];
  ...
  const allUsers = [
    ...adminUsers.map((u) => ({ ...u, teamId: null, password: hashedPassword })),
    ...btcFstyleUsers.map((u) => ({ ...u, teamId: null, password: hashedPassword })),
    ...shiroMembers.map((u) => ({ ...u, role: 'MEMBER' as const, teamId: shiro.id, password: hashedPassword })),
    ...apexMembers.map((u) => ({ ...u, role: 'MEMBER' as const, teamId: apex.id, password: hashedPassword })),
    ...slattMembers.map((u) => ({ ...u, role: 'MEMBER' as const, teamId: slatt.id, password: hashedPassword })),
    ...antiMembers.map((u) => ({ ...u, role: 'MEMBER' as const, teamId: anti.id, password: hashedPassword })),
  ];
  ...
  const awardData = [
    { name: 'Quán Quân', type: 'AUTO' as const, winnerType: 'TEAM' as const, quantity: 1, prize: '1.500.000 VND + Bảng khen + Giấy chứng nhận', displayOrder: 1 },
    { name: 'Á Quân', type: 'AUTO' as const, winnerType: 'TEAM' as const, quantity: 1, prize: '1.000.000 VND + Bảng khen + Giấy chứng nhận', displayOrder: 2 },
    { name: 'Khuyến Khích', type: 'AUTO' as const, winnerType: 'TEAM' as const, quantity: 2, prize: '500.000 VND + Bảng khen + Giấy chứng nhận', displayOrder: 3 },
    { name: 'Yêu Thích', type: 'MANUAL' as const, winnerType: 'TEAM' as const, quantity: 1, prize: 'Bảng khen + Giấy chứng nhận', displayOrder: 4 },
    { name: 'Kỹ Thuật', type: 'MANUAL' as const, winnerType: 'TEAM' as const, quantity: 1, prize: 'Chứng nhận + Quà', displayOrder: 5 },
    { name: 'Biên Đạo', type: 'MANUAL' as const, winnerType: 'TEAM' as const, quantity: 1, prize: 'Chứng nhận + Quà', displayOrder: 6 },
    { name: 'Trưởng Nhóm', type: 'MANUAL' as const, winnerType: 'INDIVIDUAL' as const, quantity: 1, prize: 'Chứng nhận + Quà', displayOrder: 7 },
    { name: 'Phong Cách', type: 'MANUAL' as const, winnerType: 'TEAM' as const, quantity: 1, prize: 'Chứng nhận + Quà', displayOrder: 8 },
    { name: 'Nỗ Lực', type: 'MANUAL' as const, winnerType: 'INDIVIDUAL' as const, quantity: 4, prize: 'Chứng nhận + Quà', displayOrder: 9 },
    { name: 'Tri Ân Mentor', type: 'MANUAL' as const, winnerType: 'INDIVIDUAL' as const, quantity: 13, prize: 'Chứng nhận + Quà', displayOrder: 10 },
  ];
